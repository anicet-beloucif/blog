    ...
    <main class="flex a_center column j_around" id="main_connexion">
        ...
        <form action="profil.php" method="POST" id="formulaire_edition" class="flex a_center column j_around">
            ...
            <?php
                ...
                if (isset($_POST['submit']) && !empty($_POST['newlogin']) && !empty($_POST['newmail']) && !empty($_POST['newpassword'])&& !empty($_POST['confpassword'])) 
			{
   
    $email1 = $_POST['newmail'];
    $password1 = $_POST['newpassword'];
    $login1 = $_POST['newlogin'];
    $req2= mysqli_query($bdd,"UPDATE utilisateurs SET login='$login1', email='$email1', password='$password1' WHERE  id = $id ");
	$res2= mysqli_fetch_all($req2,MYSQLI_ASSOC);
	$login = $res[0]['login'];
    $email = $res[0]['email'];
    $password = $res[0]['password'];
} else { echo "Merci de remplir à nouveau chaque champ";}
